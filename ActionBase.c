	...
	*/	
	...
	void WriteToContext(ParamsWriteContext ctx, ActionData action_data)
	{
		...
		if (HasTarget() && !IsUsingProxies())
		{
			...
			GetGame().Chat("Target Object " + action_data.m_Target.GetObject().ToString(), "colorAction");
			ctx.Write(targetObject);
			targetParent = action_data.m_Target.GetParent();
			GetGame().Chat("Target Parent " + action_data.m_Target.GetParent().ToString(), "colorAction");
			ctx.Write(targetParent);
			componentIndex = action_data.m_Target.GetComponentIndex();
			GetGame().Chat("Target ComponentIndex " + action_data.m_Target.GetComponentIndex().ToString(), "colorAction");
			ctx.Write(componentIndex);
			cursorHitPos = action_data.m_Target.GetCursorHitPos();
			GetGame().Chat("Curpos " + action_data.m_Target.GetCursorHitPos().ToString(), "colorAction");
